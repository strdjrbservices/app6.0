...
const Form1025 = ({ data, allData, extractionAttempted, handleDataChange, editingField,priorSaleHistoryFields, setEditingField, highlightedSubjectFields, highlightedContractFields, highlightedSiteFields, condoCoopProjectsRows, condoForeclosureFields, subjectFields, contractFields, neighborhoodFields, siteFields, improvementsFields, salesGridRows, comparableSales, salesHistoryFields, salesComparisonAdditionalInfoFields, reconciliationFields, costApproachFields, incomeApproachFields, pudInformationFields, marketConditionsRows, marketConditionsFields, appraiserFields, supplementalAddendumFields, uniformResidentialAppraisalReportFields, appraisalAndReportIdentificationFields, imageAnalysisFields, dataConsistencyFields, formType, comparisonData, getComparisonStyle, infoOfSalesFields, loading, loadingSection, stateRequirementFields, handleStateRequirementCheck, stateReqLoading, stateReqResponse, stateReqError, handleUnpaidOkCheck, unpaidOkLoading, unpaidOkResponse, unpaidOkError, handleClientRequirementCheck, clientReqLoading, clientReqResponse, clientReqError, handleFhaCheck, fhaLoading, fhaResponse, fhaError, handleEscalationCheck, escalationLoading, escalationResponse, escalationError, manualValidations, handleManualValidation, onSubjectRevisionButtonClick, onContractRevisionButtonClick, onNeighborhoodRevisionButtonClick, onSiteRevisionButtonClick, onImprovementsRevisionButtonClick, onSalesGridRevisionButtonClick, onReconciliationRevisionButtonClick, onCostApproachRevisionButtonClick, onCertificationRevisionButtonClick, on1007RevisionButtonClick, ComparableRentAdjustments, COMPARABLE_RENTAL_DATA, SUBJECT_RENT_SCHEDULE }) => (
    <>
        ...
        <div id="comparable-rental-data" style={{ marginBottom: '1rem', marginTop: '1rem' }} className="card shadow mb-4">
            ...
            <div className="card-body p-0 table-container">
                <table className="table table-hover table-striped mb-0" style={{ fontSize: '0.8rem' }}>
                    ...
                    <tbody>
                        {COMPARABLE_RENTAL_DATA.map((feature, idx) => (
                            <tr key={idx}>
                                <td className="border border-gray-400 p-1 font-medium" style={{ minWidth: '150px' }}>{feature}</td>
        ...
        <GridInfoCard id="subject-rent-schedule" title="SUBJECT RENT SCHEDULE" fields={SUBJECT_RENT_SCHEDULE} data={data.SUBJECT_RENT_SCHEDULE} cardClass="bg-info" extractionAttempted={extractionAttempted} onDataChange={(field, value) => handleDataChange(['SUBJECT_RENT_SCHEDULE', ...field], value)} editingField={editingField} setEditingField={setEditingField} isEditable={true} allData={allData} loading={loading} loadingSection={loadingSection} manualValidations={manualValidations} handleManualValidation={handleManualValidation} />
